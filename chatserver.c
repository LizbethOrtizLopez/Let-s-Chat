...
int main(int argc, char **argv)
{
	...
	{
		DieWithError("ERROR: setsockopt fallรณ");
	}

	/* Bind */
	if (bind(listenfd, (struct sockaddr *)&serv_addr, sizeof(serv_addr)) < 0)
	{
		DieWithError("ERROR: El enlace de socket fallรณ");
	}
	...
	printf("=== Let's Chat Inicializado ===\n");
	...
	{		
		...
		{
			printf("Máximo de clientes. Rechazado. ");
...
void *handle_client(void *arg)
{
	...
	/* Elimina el cliente de la cola y el socket junto con el hilo de ejecuciรณn */
	close(cli->sock);
